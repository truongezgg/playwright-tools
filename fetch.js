...
try {
  ...
  const extracted = await page.evaluate(extractFn, { customSel: selector, selectors: SELECTORS, minLen: MIN_CONTENT_LENGTH });
  ...
  if (selector && !extracted.found) {
    ...
    const fallback = await page.evaluate(extractFn, { customSel: null, selectors: SELECTORS, minLen: MIN_CONTENT_LENGTH });
