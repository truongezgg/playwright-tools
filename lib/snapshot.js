...
/**
 * Convert accessibility JSON tree to YAML-like text.
 * Playwright's page.accessibility.snapshot() returns JSON.
 * Our parsers expect YAML text format.
 */
export function snapshotToYaml(node, depth = 0) {
  if (!node) return '';
  ...
  // URL (for links)
  if (node.url) {
    line += `${indent}  - /url: ${node.url}\n`;
  }

  // Value
  if (node.value) {
    line += `${indent}  - value: "${node.value}"\n`;
  }

  // Children
  if (node.children) {
    for (const child of node.children) {
      line += snapshotToYaml(child, depth + 1);
    }
  }
